...
data_file <- "Meta-analysis spreadsheet general factors to AI 221202DB.xlsx"

# Here you set the name for the tables and figures produced (without file ending)
files_out_name <- "20-general-quality-factor"
...
nested_data <- clean_data |> 
  ...
  group_by(.data[[grouping]], revised_general_factor) |> 
  filter(n() > 1) |> 
  ungroup() |> 
...
mod_formula <- if(length(mods) > 0) {
  ...
  as.formula("~ 0 + revised_general_factor")
}
...
coefs <- models |> 
  summarise(quality_factor_comp(model))


# Write excel file with all output ----------------------------------------

tabs <- list(descriptives = descs, coefficients = coefs)
